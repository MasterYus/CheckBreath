...
public class MFCC {
    ...
    public double[] process(float[] frame) {
        ...
        /*
         * cBin=frequencies of the channels in terms of ru.myus.checkbreath.util.FFT bin indices (cBin[i]
         * for the i -th channel)
         */

        // prepare filter for for melFilter
        final int cBin[] = fftBinIndices(frame.length);// same for all
    ...
    private double[] magnitudeSpectrum(float frame[]) {
        ...
        // calculate ru.myus.checkbreath.util.FFT for current frame
        fft.computeFFT(frame);
        // System.err.println("ru.myus.checkbreath.util.FFT SUCCEED");
    ...
    private int[] fftBinIndices(int samplesPerFrame) {
